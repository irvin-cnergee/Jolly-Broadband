...
//		});
			...
			public void onClick(View v) {
				if (sharedPreferences1.getString("IsFreePackage", "false")
					...
					if (Utils.isOnline(IONHome.this)) {
						...
						if(!packageDetails.getCheckForRenewal().equals("Advance Renewed, Can not renew again...")){
							Intent i = new Intent(IONHome.this, PackgedetailActivity.class);
							i.putExtra("check_intent", true);
							i.putExtra(Utils.isPhonerenew,packageDetails.getIsPhoneRenew());
							startActivity(i);
							overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);
						}else{
		...
		relativepickuprequest.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (
				        sharedPreferences1.getString("IsFreePackage", "false")
						.equalsIgnoreCase("false")) {
					if (Utils.isOnline(IONHome.this)) {
						//IONHome.this.finish();
						// Intent i = new
				...
				if (rslt.trim().equalsIgnoreCase("ok")) {
					if (mapPackageDetails != null) {
						...
						// finish();
						 ...
						 packageDetails = mapPackageDetails
								.get(selItem);
						...
						editor.putString("MyPageAdjustmentAllowed",packageDetails.getMyPageAdjustmentAllowed());
                        editor.putInt(Utils.PayU,packageDetails.getIs_PayU());
...
*/
	...
	public void onClick(View v) {
		...
		if(v==itemShare){
			...
			startActivity(Intent.createChooser(share, "Jolly Broadband  App"));
			resideMenu.closeMenu();
		}
		...
		if (Build.VERSION.SDK_INT >= 23) {
			if (checkSelfPermission(android.Manifest.permission.CALL_PHONE)
				...
				if(param.equalsIgnoreCase("C")) {
					...
					Utils utils = new Utils();
					utils.setSharedPreferences(sharedPreferences);
					String DefaultCustomerCareNo = "8929407444";
					String CustomerCareNo = "8929407444";
//                    String CustomerCareNo = sharedPreferences.getString("CustomerCareNo", "0");
